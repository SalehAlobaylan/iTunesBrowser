...
import { Bot, Pause, Play, RefreshCw, ShieldAlert, XCircle } from 'lucide-react';
        ...
        <div className="flex min-h-[92px] flex-col justify-between gap-3 rounded-md border p-3 sm:flex-row sm:items-center">
            <div className="min-w-0 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground">{episode.verdict}</span>
                </div>
                <p className="text-sm font-medium leading-5">{episode.summary}</p>
                {episode.root_cause_hint ? (
                    <p className="text-xs text-muted-foreground">{episode.root_cause_hint}</p>
                ) : null}
                ...
                {lastTransition ? (
                    <p className="text-xs text-muted-foreground">
                        Last transition: {lastTransition.transition} · {formatDate(lastTransition.at)}
                    </p>
                ) : null}
            </div>
            {canClose ? (
...
export function SystemAutopilotPanel() {
    const { data, isLoading, isFetching, refetch } = useSystemAutopilotStatus();
            ...
            <CardHeader className="flex flex-col gap-3 border-b pb-4 md:flex-row md:items-center md:justify-between">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                        {latestRunSummary ?? 'CMS-owned incident detection and containment ledger.'}
                    </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <Badge variant={data?.state === 'safe_auto' ? 'success' : 'secondary'}>
                        {data?.state ?? (isLoading ? 'loading' : 'off')}
                    </Badge>
                    {data?.latest_run ? (
                        <Badge variant={latestRunReportsMissingEpisode ? 'warning' : 'outline'}>
                            {latestRunReportsMissingEpisode
                                ? 'episode_missing'
                                : data.latest_run.headline}
                        </Badge>
                    ) : null}
                    {data?.latest_run?.status && data.latest_run.status !== 'completed' ? (
                        <Badge
                            variant={
            ...
            <CardContent className="space-y-5 p-4">
                ...
                <div className="grid gap-3 md:grid-cols-4">
                    <div className="rounded-md border p-3">
                        ...
                        <p className="text-sm font-medium">{policy?.interval_minutes ?? 10} min</p>
                    </div>
                    <div className="rounded-md border p-3">
                        <p className="text-xs text-muted-foreground">Confirm</p>
                        <p className="text-sm font-medium">{policy?.confirm_probes ?? 2} probes</p>
                    </div>
                    <div className="rounded-md border p-3">
                        <p className="text-xs text-muted-foreground">Resolve</p>
                        <p className="text-sm font-medium">{policy?.resolve_probes ?? 3} probes</p>
                    </div>
                    <div className="rounded-md border p-3">
                            ...
                            {openEpisodes.map((episode) => (
                                <IncidentRow
                                    ...
                                    onClose={(id) =>
                                        closeEpisode.mutate({
                                            id,
                                            reason: 'Closed from System Health cockpit',
                                        })
                                    }
                                />
                            ))}
