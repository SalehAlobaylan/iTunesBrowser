...
const unwrapCmsData = async <T>(
  promise: Promise<CmsEnvelope<T>>
): Promise<T> => {
...
export const getEnrichmentStats = () =>
  unwrapCmsData(
    cmsClient.get<CmsEnvelope<EnrichmentStats>>('/admin/enrichment/stats')
  );
...
export const getMissingEnrichments = (params?: MissingEnrichmentsParams) =>
  unwrapCmsData(
    cmsClient.get<CmsEnvelope<MissingEnrichmentsResponse>>(
      '/admin/enrichment/missing',
      params
    )
  );

export const getMissingEnrichmentCounts = (
  params?: Pick<MissingEnrichmentsParams, 'type' | 'status'>
) =>
  unwrapCmsData(
    cmsClient.get<CmsEnvelope<MissingEnrichmentCounts>>(
      '/admin/enrichment/missing-counts',
      params
    )
  );
...
export const triggerBatchEnrichment = (
  ...
  unwrapCmsData(
    cmsClient.post<CmsEnvelope<TriggerResultItem[]>>(
      '/admin/enrichment/trigger-batch',
      {
 ...
 */
export const triggerAllEnrichment = (
  types: string[],
  type?: string,
  max?: number
) =>
  unwrapCmsData(
    cmsClient.post<CmsEnvelope<TriggerAllResponse>>(
      '/admin/enrichment/trigger-all',
      {
        types,
        type,
        max,
...
export const getBulkEnrichStatus = () =>
  unwrapCmsData(
    cmsClient.get<CmsEnvelope<BulkEnrichStatus>>(
      '/admin/enrichment/bulk-status'
    )
  );
...
export const getEnrichmentHealth = () =>
  unwrapCmsData(
    cmsClient.get<CmsEnvelope<EnrichmentHealthResponse>>(
...
export const getEnrichmentAutopilot = () =>
  unwrapCmsData(
    cmsClient.get<CmsEnvelope<EnrichmentAutopilotStatus>>(
      '/admin/enrichment/autopilot'
    )
  );

export const updateEnrichmentAutopilotPolicy = (
  patch: Partial<EnrichmentAutopilotPolicy>
) =>
  unwrapCmsData(
...
export const runEnrichmentAutopilotNow = () =>
  unwrapCmsData(
    cmsClient.post<CmsEnvelope<EnrichmentAutopilotRunDetail>>(
      '/admin/enrichment/autopilot/run'
    )
  );
...
export const resetEnrichmentAutopilotTrust = (artifact: string) =>
  unwrapCmsData(
    cmsClient.post<CmsEnvelope<EnrichmentAutopilotStatus>>(
      '/admin/enrichment/autopilot/trust/reset',
      { artifact }
    )
  );
