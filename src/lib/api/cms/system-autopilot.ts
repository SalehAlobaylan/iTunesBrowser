...
function normalizeSystemAutopilotStatus(status: SystemAutopilotStatus): SystemAutopilotStatus {
    const policy = status.policy ?? {
        ...
        containment_disabled_for: ['news_circulation', 'media_circulation', 'media_studio'],
    };
    return {
        ...status,
        policy,
        ...
        open_episodes: Array.isArray(status.open_episodes) ? status.open_episodes : [],
        recent_episodes: Array.isArray(status.recent_episodes) ? status.recent_episodes : [],
...
export const getSystemAutopilotStatus = () =>
    unwrap(cmsClient.get<CmsEnvelope<SystemAutopilotStatus>>(`${BASE}/status`)).then(
        normalizeSystemAutopilotStatus
    );

export const getSystemAutopilotPolicy = () =>
    unwrap(cmsClient.get<CmsEnvelope<SystemAutopilotPolicy>>(`${BASE}/policy`));

export const updateSystemAutopilotPolicy = (patch: Partial<SystemAutopilotPolicy>) =>
    unwrap(cmsClient.put<CmsEnvelope<SystemAutopilotPolicy>>(`${BASE}/policy`, patch));
...
export const pauseSystemAutopilotContainment = (minutes: number) =>
    unwrap(
        cmsClient.post<CmsEnvelope<{ containment_paused_until: string | null }>>(`${BASE}/pause`, {
            minutes,
        })
    );
...
export const listSystemIncidentEpisodes = (limit = 50) =>
    unwrap(
        cmsClient.get<CmsEnvelope<{ items: SystemIncidentEpisode[] }>>(`${BASE}/episodes`, {
            limit,
        })
    );
...
export const closeSystemIncidentEpisode = (id: string, reason?: string) =>
    unwrap(
        cmsClient.post<CmsEnvelope<SystemIncidentEpisode>>(`${BASE}/episodes/${id}/close`, {
            reason,
        })
    );
...
export const listSystemAutopilotRuns = (limit = 20) =>
    unwrap(
        cmsClient.get<CmsEnvelope<{ items: SystemAutopilotRun[] }>>(`${BASE}/runs`, {
            limit,
        })
    );

export const getSystemAutopilotRun = (id: string) =>
    unwrap(cmsClient.get<CmsEnvelope<SystemAutopilotRunDetail>>(`${BASE}/runs/${id}`));
