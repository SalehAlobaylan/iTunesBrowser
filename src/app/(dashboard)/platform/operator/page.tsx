...
import { operatorRouteManifest, type OperatorRouteDescriptor } from '@/lib/operator/route-manifest';
...
function eligibleActionLabel(action: OperatorEligibleAction, locale: 'en' | 'ar'): string {
	const supplyLabels: Record<string, readonly [string, string]> = {
		...
		'source_run.verify_effect': ['Request independent verification', 'طلب تحقق مستقل'],
		...
		'pipeline.resume_exact_stage': ['Resume exact pipeline stage', 'استئناف مرحلة المعالجة الدقيقة'],
		...
		'artifact.request_llm_metadata': ['Recover LLM metadata', 'استرداد بيانات النموذج'],
		'atomization.execute_exact_parent': ['Atomize exact long parent', 'تجزئة الأصل الطويل المحدد'],
		'feed_generation.attach_verified_member': ['Attach verified generation member', 'إلحاق عنصر موثّق بالجيل'],
	};
	const localized = supplyLabels[action.key];
	return localized?.[locale === 'ar' ? 1 : 0] ?? action.localized_action_key.replace('operator.action.', '').replaceAll('_', ' ');
}
...
export default function OperatorWorkspacePage() {
  ...
  const copy = operatorTranslations[locale];
	const action = (() => {
		...
		return candidate ? { toolKey: candidate.key, label: eligibleActionLabel(candidate, locale), affectedDomains: candidate.affected_domains, targetIDs: candidate.target_ids } : undefined;
	})();
	// Investigation context can contain route or browser selection IDs, but only
	// the CMS eligibility descriptor is executable authority.
	const actionTargetIDs = eligibleActionTargetIDs(eligibleActions.find((item) => item.key === action?.toolKey));
      ...
      try {
        ...
        if (['succeeded', 'failed', 'blocked', 'cancelled'].includes(snapshot.state)) {
			if (snapshot.verified_effects && !invalidatedPlansRef.current.has(snapshot.id)) {
				...
				const affected = new Set(snapshot.verified_effects.affected_domains ?? []);
				void queryClient.invalidateQueries({ predicate: (cached) => {
					const owned = (cached.meta?.affectedDomains as string[] | undefined) ?? [];
					return owned.some((domain) => affected.has(domain)) || (affected.has(visibleContext.domain) && cached.getObserversCount() > 0);
				} });
			}
  ...
  const markRead = async (id: string) => {
    ...
    <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      ...
      <div className="grid gap-5 p-5 lg:grid-cols-[0.9fr_1.1fr]">
		<div className="space-y-3">
          ...
          {eligibleActions.length ? <select aria-label={locale === 'ar' ? 'إجراء CMS المؤهل' : 'CMS eligible action'} value={domainTool || eligibleActions[0].key} onChange={(event) => setDomainTool(event.target.value)} className="h-10 w-full rounded-md border bg-background px-3 text-sm">{eligibleActions.map((candidate) => <option key={candidate.key} value={candidate.key}>{eligibleActionLabel(candidate, locale)}</option>)}</select> : result ? <p className="text-xs text-muted-foreground">{locale === 'ar' ? 'لم تُرجع CMS إجراءً آمناً ومؤهلاً لهذا السياق.' : 'CMS found no safe eligible action for this context.'}</p> : null}
          ...
          <Input aria-label={copy.exactActionTarget} dir="ltr" value={explicitTarget} onChange={(event) => { setStoredContext(undefined); setExplicitTarget(event.target.value); }} placeholder={locale === 'ar' ? 'معرّف سياق اختياري' : 'Optional investigation context ID'} />
		  <p className="text-xs leading-5 text-muted-foreground">{locale === 'ar' ? 'هذا المعرّف سياق للتحقيق فقط. لا يصبح هدف تنفيذ إلا إذا أعادته CMS ضمن الإجراء المؤهل.' : 'This ID is investigation context only. It becomes executable only when CMS returns it in an eligible-action descriptor.'}</p>
        </div>
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
      ...
      <aside className="space-y-5 lg:sticky lg:top-4 lg:self-start" aria-label={locale === 'ar' ? 'لوحات مساحة المشغّل' : 'Operator workspace panels'}>
        <OperatorGovernancePanel locale={locale} controls={controls} loading={governanceLoading} controlLabel={(key) => localizedControlLabel(key, locale)} onRefresh={() => { void loadGovernance(); }} />
        <OperatorThreadsPanel locale={locale} threads={threads} activeThreadID={activeThreadID} onSelect={(thread) => { setActiveThreadID(thread.id); setLocale(thread.locale); }} onToggleLocale={(thread) => { void updateThreadLocale(thread.id, thread.locale === 'ar' ? 'en' : 'ar'); }} onDelete={(id) => { void deleteThread(id); }} onCreate={() => { void createThread('', locale); }} />
        <OperatorInboxPanel locale={locale} inbox={inbox} lifecycleLabel={(state) => localizedLifecycle(state, locale)} onOpen={(id) => { void markRead(id); void loadEvents(id); }} onRefresh={() => { void loadInbox(); }} />
      </aside>
    </div>
  </main>;
}
